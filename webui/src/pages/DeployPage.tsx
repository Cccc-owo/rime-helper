...
export default function DeployPage() {
  ...
  return (
    <div class="page">
      ...
      <SurfaceSection title="概览">
        <div class="deploy-summary-grid">
          <div class="deploy-summary-item"><span class="label">检测到应用</span><b>{state.apps.length}</b></div>
          <div class="deploy-summary-item"><span class="label">启用资源数</span><b>{state.snapshot?.summary.enabledResourceCount ?? 0}</b></div>
          <div class="deploy-summary-item"><span class="label">上次下载完成</span><b>{formatTs(state.snapshot?.download.updatedAt ?? '0')}</b></div>
          <div class="deploy-summary-item"><span class="label">上次同步完成</span><b>{formatTs(state.snapshot?.deploy.lastSuccessAt ?? '0')}</b></div>
          <div class="deploy-summary-item"><span class="label">同步目标</span><b>{state.snapshot?.targetApps ? '已指定' : '全部应用'}</b></div>
          <div class="deploy-summary-item"><span class="label">后端同步状态</span><b>{deployStateText()}</b></div>
        </div>
